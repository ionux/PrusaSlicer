...
int CLI::run(int argc, char **argv)
{
    ...
    catch (const std::runtime_error &ex)
    {
        ...
        std::string text = std::string("An error occured while setting up locale.\n") +
            (
                               ...
                               "You may need to reconfigure the missing locales, likely by running "
                               "the \"locale-gen\" and \"dpkg-reconfigure locales\" commands.\n"
#endif
    ...
    bool start_as_gcodeviewer =
        ...
        false;
#else
        // On Unix systems, the prusa-slicer binary may be symlinked to give the application a
        // different meaning.
    ...
    const ForwardCompatibilitySubstitutionRule config_substitution_rule =
        m_config
            .option<ConfigOptionEnum<ForwardCompatibilitySubstitutionRule>>(
    ...
    for (auto const &file : load_configs)
    {
        ...
        {
            boost::nowide::cout << "The following configuration values were substituted when "
                                   "loading \" << file << \":\n";

            for (const ConfigSubstitution &subst : config_substitutions)
            {
                boost::nowide::cout << "\tkey = \"" << subst.opt_def->opt_key << "\"\t loaded = \""
                                    << subst.old_value << "\tsubstituted = \""
                                    << subst.new_value->serialize() << "\"\n";
            }
        }
    ...
    if (it != m_actions.end())
    {
        ...
        if (semver.has_value() && (*semver) >= opengl_minimum)
        {
            ...
            if (std::find(
                    Slic3r::GUI::OpenGLVersions::core.begin(),
                    Slic3r::GUI::OpenGLVersions::core.end(),
                    std::make_pair(opengl_version.first, opengl_version.second)
                ) == Slic3r::GUI::OpenGLVersions::core.end())
            {
                opengl_version = {0, 0};
    ...
    for (const std::string &s :
         {"opengl-version", "opengl-compatibility", "opengl-debug", "opengl-aa", "gcodeviewer"})
    {
    ...
    if (!start_as_gcodeviewer)
    {
        for (const std::string &file : m_input_files)
        {
            ...
            try
            {
                ...
                {
                    // When loading an AMF or 3MF, config is imported as well, including the printer
                    // technology.
                    DynamicPrintConfig config;
                    ...
                    {
                        boost::nowide::cout << "The following configuration values were "
                                               "substituted when loading \" << file << \":\n";

                        for (const ConfigSubstitution &subst : config_substitutions.substitutions)
                        {
                            boost::nowide::cout << "\tkey = \"" << subst.opt_def->opt_key
                ...
                // If model for slicing is loaded from 3mf file, then its geometry has to be used
                // and arrange couldn't be apply for this model.
                if ((boost::algorithm::iends_with(file, ".3mf") ||
                     boost::algorithm::iends_with(file, ".zip")) &&
                    !m_config.opt_bool("dont_arrange"))
                {
                    // So, check a state of "dont_arrange" parameter and set it to true, if its
                    // value is false.
    ...
    for (auto const &opt_key : m_transforms)
    {
        ...
        else if (opt_key == "duplicate")
        {
            for (auto &model : m_models)
            {
                const bool all_objects_have_instances =
                    std::none_of(model.objects.begin(), model.objects.end(), [](ModelObject *o) {
                        return o->instances.empty();
                    });
        ...
        else if (opt_key == "duplicate_grid")
        {
            ...
            for (auto &model : m_models)
            {
                ...
                model.duplicate_objects_grid(x, y, (distance > 0) ? distance : 6);
            }
        }
        else if (opt_key == "center")
        {
            ...
            for (auto &model : m_models)
            {
                ...
                model.center_instances_around_point(
                    m_config.option<ConfigOptionPoint>("center")->value
                );
                // this affects volumes:
                // FIXME Vojtech: Who knows why the complete model should be aligned with Z as a
                // single rigid body? model.align_to_ground();
                BoundingBoxf3 bbox;
                for (ModelObject *model_object : model.objects)
                    // We are interested into the Z span only, therefore it is sufficient to measure
        ...
        else if (opt_key == "ensure_on_bed")
        {
            ...
            for (auto &model : m_models) {
                ...
                for (size_t i = 0; i < num_objects; ++i) {
                    ...
                    Cut cut(
                        model.objects.front(), 0,
                        Geometry::translation_transform(m_config.opt_float("cut") * Vec3d::UnitZ()),
                        ModelObjectCutAttribute::KeepLower | ModelObjectCutAttribute::KeepUpper |
    ...
    for (auto const &opt_key : m_actions)
    {
        if (opt_key == "help") {
            ...
            if (opt_key == "export_gcode" && printer_technology == ptSLA) {
                boost::nowide::cerr << "error: cannot export G-code for an FFF configuration"
                                    << std::endl;
                return 1;
            } else if (opt_key == "export_sla" && printer_technology == ptFFF) {
                boost::nowide::cerr << "error: cannot export SLA slices for a SLA configuration"
                                    << std::endl;
                return 1;
            }
            // Make a copy of the model if the current action is not the last action, as the model
            // may be modified by the centering and such.
            Model model_copy;
            ...
            for (Model &model_in : m_models) {
                ...
                if (print->empty())
                    boost::nowide::cout << "Nothing to print for " << outfile
                                        << " . Either the print is empty or no object is fully "
                                           "inside the print volume."
                                        << std::endl;
                else
                    try {
                        ...
                        if (printer_technology == ptFFF) {
                            ...
                            // We need to finalize the filename beforehand because the export
                            // function sets the filename inside the zip metadata
                        ...
                        if (outfile != outfile_final) {
                            if (Slic3r::rename_file(outfile, outfile_final)) {
                                boost::nowide::cerr << "Renaming file " << outfile << " to "
                                                    << outfile_final << " failed" << std::endl;
                                return 1;
                            }
                        ...
                        boost::nowide::cout << "Slicing result exported to " << outfile
                                            << std::endl;
                ...
                /*
                                ...
                                double duration { std::chrono::duration_cast<second_>(clock_::now()
                   - t0).count() }; boost::nowide::cout << std::fixed << std::setprecision(0)
    ...
    if (start_gui) {
        ...
        if (!(display && *display)) {
            // DISPLAY not set.
            boost::nowide::cerr << "DISPLAY not set, GUI mode not available." << std::endl
                                << std::endl;
       ...
       // No GUI support. Just print out a help.
        this->print_help(false);
        // If started without a parameter, consider it to be OK, otherwise report an error code (no
        // action etc).
        return (argc == 0) ? 0 : 1;
#endif // SLIC3R_GUI
    }
...
bool CLI::setup(int argc, char **argv) {
    {
        ...
        if (loglevel != nullptr) {
            if (loglevel[0] >= '0' && loglevel[0] <= '9' && loglevel[1] == 0)
                ...
                boost::nowide::cerr << "Invalid SLIC3R_LOGLEVEL environment variable: " << loglevel
                                    << std::endl;
        }
    }
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into PrusaSlicer process (for example
    // Nahimic, see GH #5573). We hope that if a DLL is being injected into a PrusaSlicer process,
    // it happens at the very start of the application, thus we shall detect them now.
    ...
    // Resources are set to the 'Architecture-independent (shared) data', typically /usr/share or
    // /usr/local/share
    ...
    // FIXME Validating at this stage most likely does not make sense, as the config is not fully
    // initialized yet.
    std::string validity = m_config.validate();

    // Initialize with defaults.
    for (const t_optiondef_map *options :
         {&cli_actions_config_def.options, &cli_transform_config_def.options,
          &cli_profiles_sharing_config_def.options, &cli_misc_config_def.options})
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const {
...
#endif /* SLIC3R_GUI */
        << std::endl
        << "https://github.com/prusa3d/PrusaSlicer" << std::endl
        << std::endl
    ...
    boost::nowide::cout << std::endl << "Transform options:" << std::endl;
    cli_transform_config_def.print_cli_help(boost::nowide::cout, false);

    boost::nowide::cout << std::endl << "Other options:" << std::endl;
    ...
    if (include_print_options) {
        ...
        print_config_def.print_cli_help(
            boost::nowide::cout, true,
...
std::set<std::string> query_options = {"printer-profile"};
...
bool CLI::processed_profiles_sharing() {
    ...
    for (auto const &opt_key : m_profiles_sharing) {
        ...
        /*
                    ...
                    if (!m_config.has("printer_model") || !m_config.has("printer_variant")) {
                        boost::nowide::cerr << "error in '" << opt_key << "' : this action requires
           set 'printer-model' and 'printer-variant' options" << std::endl; break;
                    }
                    ret = Slic3r::get_json_printer_profiles(m_config.opt_string("printer_model"),
           m_config.opt_string("printer_variant"));

                    if (ret.empty())
                        boost::nowide::cerr <<  "Printer_model '" <<
           m_config.opt_string("printer_model") <<
                                                "' with printer_variant '" <<
           m_config.opt_string("printer_variant") <<
                                                "' wasn't found among installed printers." <<
           std::endl << "Or the request can be wrong." << std::endl;
                }
        */
        else if (opt_key == "query-print-filament-profiles") {
            if (!m_config.has("printer-profile")) {
                boost::nowide::cerr << "error: this action requires set printer-preset option"
                                    << opt_key << std::endl;
                break;
            }
    ...
    if (cmdline_param.empty()) {
        ...
        // if we were supplied a directory, use it and append our automatically generated filename
        boost::filesystem::path cmdline_path(cmdline_param);
        boost::filesystem::path proposed_path = boost::filesystem::path(Slic3r::resources_dir()) /
            "out.json";
        if (boost::filesystem::is_directory(cmdline_path))
            proposed_path = (cmdline_path / proposed_path.filename());
...
bool CLI::check_and_load_input_profiles(PrinterTechnology &printer_technology) {
    Slic3r::DynamicPrintConfig config = {};
    std::string ret = Slic3r::load_full_print_config(
        m_config.opt_string("print-profile"),
        m_config.option<ConfigOptionStrings>("material-profile")->values,
        m_config.opt_string("printer-profile"), config, printer_technology
    );
    if (!ret.empty()) {
    ...
    if (printer_technology == ptUnknown)
        printer_technology = other_printer_technology;
             ...
             other_printer_technology != ptUnknown) {
        boost::nowide::cerr << "Mixing configurations for FFF and SLA technologies" << std::endl;
        return false;
    }
...
// __has_feature() is used later for Clang, this is for compatibility with other compilers (such as
// GCC and MSVC)
...
extern "C" {
// Enable printing stacktrace by default. It can be disabled by running PrusaSlicer with
